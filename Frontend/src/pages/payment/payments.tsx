...
import { API_BASE_URL, authHeaders, storage } from '../../lib';
...
export default function PaymentsPage() {
    ...
    const canSeeRecaudacion = useMemo(() => {
        if (!user) return false;
        const role = user.role?.toLowerCase() || '';
        return ['admin', 'administrador', 'gerente', 'secretaria'].includes(role);
    }, [user]);
    ...
    const [collections, setCollections] = useState<AdvisorCollection[]>([]);
    ...
    const fetchRoute = async () => {
        try {
            const { data } = await axios.get(`${API_BASE_URL}/loans/daily-route/`, { headers: authHeaders() });
            ...
            data.forEach((loan: APILoan) => {
                if (!loan) return;
                // The backend already filters for disbursed loans and those due today
                const clientId = String(loan.cliente_id || loan.cliente_dpi || loan.id);
                if (!clientsMap.has(clientId)) {
                    clientsMap.set(clientId, {
                        id: clientId,
                        name: loan.cliente_nombre,
                ...
                const principal = Number(loan.monto_solicitado) || 0;
                const interest = Number(loan.interes) || 0;
                const cuotas = Number(loan.numero_cuotas) || 1;
                const total = principal + (principal * interest / 100);
                const expectedQuota = total / cuotas;

                client.activeLoans.push({
                    id: String(loan.id),
                    type: loan.plan_nombre || 'Préstamo',
                    total: total,
                    balance: total,
                    expectedQuota: Math.round(expectedQuota * 100) / 100
                });
            });
    ...
    const fetchCollections = async () => {
        try {
            const { data } = await axios.get(`${API_BASE_URL}/loans/collections-summary/`, { headers: authHeaders() });
            if (data && Array.isArray(data)) {
                setCollections(data);
            } else {
                setCollections([]);
    ...
    const pendingCollections = useMemo(() => collections.filter(c => c.status === 'pending' || c.status === 'rejected'), [collections]);
    ...
    const handleReject = async (id: string) => {
        if (!window.confirm("¿Está seguro de que desea rechazar esta recaudación?")) return;
        try {
            await axios.post(`${API_BASE_URL}/loans/bulk-reject-collections/`, {
                advisor_ids: [id]
            }, { headers: authHeaders() });
            ...
            alert("Hubo un error al rechazar la recaudación.");
        }
    };
    const handleEnviarCaja = async () => {
        try {
            const advisorIds = validatedCollections.map(c => c.id);
            await axios.post(`${API_BASE_URL}/loans/bulk-validate-collections/`, {
            ...
            fetchCollections(); // Final refresh
        } catch (err) {
            console.error("Error sending to caja", err);
            alert("Hubo un error al enviar a caja. Por favor intente de nuevo.");
        }
    };
    ...
    const morosos = useMemo(() => {
        ...
        return clients.filter(c => {
            if (!c || c.status !== 'completed' || !c.paidAmounts || !c.activeLoans) return false;
            return c.activeLoans.some(loan => (c.paidAmounts![loan.id] ?? 0) < (loan.expectedQuota || 0));
        });
    }, [clients]);

    const stats = useMemo(() => {
        if (!clients || !Array.isArray(clients)) return { total: 0, visited: 0, percentage: 0, expectedMoney: 0, collectedMoney: 0, moneyPercentage: 0, totalLoans: 0 };
        ...
        clients.forEach(c => {
            if (!c || !c.activeLoans) return;
            c.activeLoans.forEach(loan => { expectedMoney += (loan.expectedQuota || 0); totalLoans++; });
            if (c.status === 'completed' && c.paidAmounts) {
                Object.values(c.paidAmounts).forEach(amount => { collectedMoney += (amount || 0); });
            }
        });
    ...
    const handleRegisterPayment = async (client: Client) => {
        ...
        try {
            ...
            for (const loan of client.activeLoans) {
                const amount = getNumericValue(loan.id);
                if (amount > 0) {
                    await axios.post(`${API_BASE_URL}/loans/payments/`, {
                        prestamo_id: loan.id,
                        monto_pagado: amount,
                        tipo_pago: 'Efectivo',
                        observaciones: 'Cobro en ruta'
                    }, { headers: authHeaders() });
                    numericAmounts[loan.id] = amount;
                }
            }
            ...
            setTimeout(() => {
                setPaymentSuccessId(null);
                fetchRoute(); // Refresh the list - client will disappear if fully paid for today
                fetchCollections(); // Refresh collections summary
            }, 1500);
        } catch (error) {
    ...
    return (
        <div className="payments-container w-100 h-100 d-flex flex-column overflow-auto custom-scrollbar">
                    ...
                    <div className="d-flex justify-content-between align-items-start mb-4">
                        <div>
                            <div className="d-flex align-items-center gap-2 mb-3">
                                <span className="payments-date-badge"><CalendarIcon /> HOY, {new Date().toLocaleDateString('es-GT', { day: 'numeric', month: 'long' }).toUpperCase()}</span>
                            </div>
            ...
            {/* ================= RECAUDACIÓN TAB ================= */}
            {activeTab === 'recaudacion' && canSeeRecaudacion && (
                                            ...
                                            <div className="d-flex align-items-center gap-3" style={{ flex: 1, minWidth: 0 }}>
                                                ...
                                                <div style={{ minWidth: 0 }}>
                                                    <div className="d-flex align-items-center gap-2">
                                                        ...
                                                        {col.status === 'rejected' && <span className="badge bg-danger" style={{ fontSize: '9px' }}>RECHAZADO</span>}
                                                    </div>
